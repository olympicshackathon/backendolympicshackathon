...
const debug = require('debug')('olympics:auth-router');
...
authRouter.post('/api/signup', json(), (req, res, next) => {
    ...
    user.generatePasswordHash(password)
        ...
        .then(token => {
            res.cookie('Olympic-Token', token, {maxAge: 604800000});
        })
        .catch(next);
});
...
authRouter.get('/api/signin', basicAuth, (req, res, next) => {
    ...
    let currentUser = User.findOne({ username: req.auth.username })
        ...
        .then(user => user.camparePasswordHash(req.auth.password))
...
authRouter.get('/api/signup/usernames/:username', (req, res, next) => {
    User.findOne({ username: req.params.username })
        .then({ username: req.params.username })
        .then(user => {
            if (!user)
